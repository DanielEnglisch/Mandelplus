...
Color MandelbrotRenderer::getColor(const unsigned int i, mpreal  r, mpreal  c) {
	const mpreal size{ sqrt(r * r + c * c) };
	const mpreal smoothed{ log(log(size) * ONE_OVER_LOG2) * ONE_OVER_LOG2 };
...
ManVal MandelbrotRenderer::getMandelbrotValue(const int x, const int y) {
	...
	mpreal a{ map(x, 0, width, -zoom, zoom) + dx };
	mpreal b{ map(y, 0, height, -zoom, zoom) + dy };

	const mpreal initialA{ a };
	const mpreal initialB{ b };
